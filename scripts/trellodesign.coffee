...
#   SOBRRBOT_TRELLO_DESIGN_IDEA_LIST - The list ID that you'd like to create cards for
#
# Commands:
#   hubot trello design card <name> - Create a new Trello card in UI-UX/Design board, Bugs list
#
# Notes:
...
module.exports = (robot) ->
  robot.respond /trello design (.*)/i, (msg) ->
    ...
    if not process.env.SOBRRBOT_TRELLO_DESIGN_IDEA_LIST
      msg.send "Error: Trello list ID is not specified"
    if not (process.env.SOBRRBOT_TRELLOIDEA_KEY and process.env.SOBRRBOT_TRELLOIDEA_TOKEN and process.env.SOBRRBOT_TRELLO_DESIGN_IDEA_LIST)
      msg.send "ERROR1"
      return
...
createCard = (msg, cardName) ->
  ...
  t.post "/1/cards", {name: cardName, idList: process.env.SOBRRBOT_TRELLO_DESIGN_IDEA_LIST}, (err, data) ->
    if err
