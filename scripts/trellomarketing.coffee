...
module.exports = (robot) ->
  robot.respond /trello marketing (.*)/i, (msg) ->
    ...
    if not process.env.SOBRRBOT_TRELLOIDEA_LIST
      msg.send "Error: Trello list ID is not specified"
    if not (process.env.SOBRRBOT_TRELLOIDEA_KEY and process.env.SOBRRBOT_TRELLOIDEA_TOKEN and process.env.SOBRRBOT_TRELLOIDEA_LIST)
      msg.send "ERROR1"
      return
...
createCard = (msg, cardName) ->
  ...
  t.post "/1/cards", {name: cardName, idList: process.env.SOBRRBOT_TRELLOIDEA_LIST}, (err, data) ->
    if err
